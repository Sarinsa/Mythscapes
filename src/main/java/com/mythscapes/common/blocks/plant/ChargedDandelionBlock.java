...
public class ChargedDandelionBlock extends MythFlowerBlock {
    ...
    public boolean isMature(BlockState blockState) {
        return blockState.get(AGE) == 9;
    }

    private void attemptSpread(World world, BlockPos pos) {
    ...
    public void tick(BlockState state, ServerWorld world, BlockPos pos, Random rand) {
        if (!this.isMature(state)) {
            if (rand.nextInt(35) == 0) {
                ...
                world.setBlockState(pos, state.with(AGE, ++currentAge));
            }
        }
    }
