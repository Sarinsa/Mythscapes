...
public class MythBoatEntity extends BoatEntity {
    ...
    private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(MythBoatEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(MythBoatEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(MythBoatEntity.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(MythBoatEntity.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(MythBoatEntity.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(MythBoatEntity.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(MythBoatEntity.class, DataSerializers.VARINT);
    ...
    private MythBoatEntity.Status status;
    private MythBoatEntity.Status previousStatus;
    ...
    public MythBoatEntity(EntityType<? extends MythBoatEntity> entityType, World world) {
        super(entityType, world);
    }

    public MythBoatEntity(World world) {
        super(MythEntities.MYTH_BOAT.get(), world);
    ...
    public void tick() {
        ...
        if (this.status != MythBoatEntity.Status.UNDER_WATER && this.status != MythBoatEntity.Status.UNDER_FLOWING_WATER) {
     ...
     */
    private MythBoatEntity.Status getBoatStatus() {
        MythBoatEntity.Status boatentity$status = this.getUnderwaterStatus();
        if (boatentity$status != null) {
            this.waterLevel = this.getBoundingBox().maxY;
            return boatentity$status;
        } else if (this.checkInWater()) {
            return MythBoatEntity.Status.IN_WATER;
        } else {
            float f = this.getBoatGlide();
            if (f > 0.0F) {
                this.boatGlide = f;
                return MythBoatEntity.Status.ON_LAND;
            } else {
                return MythBoatEntity.Status.IN_AIR;
            }
        }
    }
    ...
    public float getWaterLevelAbove() {
        ...
        int l = MathHelper.ceil(boundingBox.maxY - this.lastYd);
        int i1 = MathHelper.floor(boundingBox.minZ);
        int j1 = MathHelper.ceil(boundingBox.maxZ);
     ...
     */
    @Nullable
    private MythBoatEntity.Status getUnderwaterStatus() {
        AxisAlignedBB axisalignedbb = this.getBoundingBox();
                    ...
                    for(int i2 = i1; i2 < j1; ++i2) {
                        ...
                        if (ifluidstate.isTagged(FluidTags.WATER) && d0 < (double)((float)blockpos$pooledmutable.getY() + ifluidstate.getActualHeight(this.world, blockpos$pooledmutable))) {
                            if (!ifluidstate.isSource()) {
                                MythBoatEntity.Status boatentity$status = MythBoatEntity.Status.UNDER_FLOWING_WATER;
                                return boatentity$status;
                            }
        ...
        return flag ? MythBoatEntity.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion() {
        ...
        double d2 = 0.0D;
        this.momentum = 0.05F;
        if (this.previousStatus == MythBoatEntity.Status.IN_AIR && this.status != MythBoatEntity.Status.IN_AIR && this.status != MythBoatEntity.Status.ON_LAND) {
            ...
            this.lastYd = 0.0D;
            this.status = MythBoatEntity.Status.IN_WATER;
        }
        else {

            if (this.status == MythBoatEntity.Status.IN_WATER) {
                d2 = (this.waterLevel - this.getPosY()) / (double) this.getHeight();
                this.momentum = 0.9F;
            }
            else if (this.status == MythBoatEntity.Status.UNDER_FLOWING_WATER) {
                d1 = -7.0E-4D;
                this.momentum = 0.9F;
            }
            else if (this.status == MythBoatEntity.Status.UNDER_WATER) {
                d2 = (double) 0.01F;
                this.momentum = 0.45F;
            }
            else if (this.status == MythBoatEntity.Status.IN_AIR) {
                this.momentum = 0.9F;
            }
            else if (this.status == MythBoatEntity.Status.ON_LAND) {
    ...
    protected void writeAdditional(CompoundNBT compound) {
        compound.putString("Type", this.getMythBoatType().getName());
    }
    ...
    protected void readAdditional(CompoundNBT compound) {
        if (compound.contains("Type", 8)) {
            this.setBoatType(MythBoatEntity.Type.getTypeFromString(compound.getString("Type")));
        }
    }
        ...
        if (!this.isPassenger()) {
            if (onGroundIn) {
                if (this.fallDistance > 3.0F) {
                    if (this.status != MythBoatEntity.Status.ON_LAND) {
    ...
    @OnlyIn(Dist.CLIENT)
    public void updateInputs(boolean leftDown, boolean rightDown, boolean forwardDown, boolean backDown) {
        this.leftInputDown = leftDown;
        this.rightInputDown = rightDown;
        this.forwardInputDown = forwardDown;
        this.backInputDown = backDown;
    }
