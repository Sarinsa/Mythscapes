...
public class MythAdvancementProvider extends AdvancementProvider {

    private final DataGenerator dataGenerator;
    private final Gson GSON = new GsonBuilder().setPrettyPrinting().create();
    private final ImmutableList<Consumer<Consumer<Advancement>>> advancements;

    public MythAdvancementProvider(DataGenerator dataGenerator, Consumer<Consumer<Advancement>>... consumers) {
        super(dataGenerator);
    ...
    private static Path getPath(Path pathIn, Advancement advancementIn) {
        return pathIn.resolve("data/" + Mythscapes.MODID + "/advancements/" + advancementIn.getId().getPath() + ".json");
    }
}
